...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; register markup template

(define-public (register-markup path mup . extra) #f)
(define-public (get-markup path . def) #f)
(let ((mups (tree-create 'markup)))
  (set! register-markup (lambda (path mup . extra)
                          (let ((parser #f)
                                (location #f))
                            (for-each
                             (lambda (e)
                               (cond
                                ((ly:lily-parser? e) (set! parser e))
                                ((ly:input-location? e) (set! location e))
                                )) extra)
                            (if (markup? mup)
                                (tree-set! mups path mup)
                                (ly:warning "no markup! ~A" mup)
                                ))))
  (set! get-markup (lambda (path . extra)
                     (let ((parser #f)
                           (location #f)
                           (def (markup))
                           (mup (tree-get mups path)))
                       (for-each
                        (lambda (e)
                          (cond
                           ((markup? e) (set! def e))
                           ((ly:parser? e) (set! parser e))
                           ((ly:input-location? e) (set location e))
                           )) extra)
                       (cond
                        ((markup? mup) mup)
                        ((and (> (length def) 0) (markup? (car def))) (car def))
...
(define-markup-command (copyright layout props)()
  (let ((mup (get-markup-producer copyright-markup)))
    (interpret-markup layout props (mup layout props (list)))))
(register-markup-producer copyright-markup
  (lambda (layout props args) (markup #:char 169 (if (not (chain-assoc-get 'header:copyright-no-year props #f))(markup #:year) "") #:fromproperty 'header:copyright )))
...
(define-public (markup->lily mup)
  (let ((str (string? mup)))
    ...
    (if str (string-append "\"" mup "\"")
        (string-append "\\markup " (markup-to-lily mup)))
    ))
