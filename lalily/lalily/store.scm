                                    ...
                                    (let ((oplist (tree-collect templates piece (stack-create)))
                                      ...
                                      (for-each (lambda (p)
                                                  (let* ((o (if (pair? p)(cdr p) '()))
                                                    ...
                                                    (if (> (length head))(set! opts (assoc-set opts 'header head)))
                                                    )) (reverse oplist))
...
(define-public (set-default-header piece field value)
  (let* ((tmpl (get-default-template piece))
    ...
    (set! header (acons field value (assoc-remove header field)))
    (set! opts (acons 'header header (assoc-remove opts 'header)))
