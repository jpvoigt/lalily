...
(define-public (display-template-ref) #f)
                      ...
                      (if (and (list? t) (> (length t) 0))
                          (let ((tt (tree-get template-doc t))
                            ...
                            (if (tree? tt)
                                ...
                                (begin (set! tt (tree-create 'template-ref)) (tree-set! template-doc t tt)))
                            (if (not val) (set! val 'need))
                            (tree-set! tt pp val)
                            ))
                           ...
                           (if (and (list? t) (> (length t) 0))
                               (let ((tt (tree-get template-doc t))
                                 ...
                                 (if (tree? tt)
                                     (set! val (tree-get tt pp))
                                     (begin (set! tt (tree-create 'template-ref)) (tree-set! template-doc t tt)))
                                 (if (not val) (set! val (if (ly:music? defm) 'deep-optional 'deep)))
                                 (tree-set! tt pp val)
                                 ))
                           (if (ly:music? p) (ly:music-deep-copy p)
                               (if (ly:music? defm)
