...
(let ((music-tree (tree-create 'music)))
  ...
  (set! has-music? (lambda (path) (ly:music? (tree-get music-tree path))))

  (set! display-music-pieces (lambda ()
                               (tree-display music-tree `(vformat . ,(lambda (v)
                                                                       (if (ly:music? v) (let ((mom (ly:music-length v))) (format "~A/~A" (ly:moment-main-numerator mom) (ly:moment-main-denominator mom))) "*"))) )))
  )
